...
def camel_product_list(update, context):
    ...
    if not products:
        ...
        while i < len(products):
            if (i % 12) == 0:
                info += " - " + ProductService().get_name(products[i][0]) + ":\n"  # name
...
def add_set_url(update, context):
    ...
    if not ProductService().check_name(product_details["name"]):
...
def remove_update_db(update, context):
    ...
    if ProductService().check_name(name):
        res = ProductService().delete(name)
...
def change_set_old_name(update, context):
    if ProductService().count_amazon_product() != 0:
...
def change_set_new_name(update, context):
    ...
    elif ProductService().check_name(product_name["old_name"]):
...
def change_update_db(update, context):
    ...
    elif not ProductService().check_name(product_name["new_name"]):
        res = ProductService().update_name(product_name["old_name"], product_name["new_name"])
...
def get_url_set_name(update, context):
    if ProductService().count_amazon_product() > 1:
    ...
    elif ProductService().count_amazon_product() == 1:
        name = ProductService().get_first_name()

        if ProductService().check_name(name) and name:
            url = ProductService().get_url_by_name(name)
...
def get_url_search_db(update, context):
    ...
    if ProductService().check_name(name):
        res = ProductService().get_url_by_name(name)
...
def check_price(context):
    ...
    amazon_product = ProductService().get_amazon_urls()
...
def main():
    ...
    job.run_repeating(check_price, interval=((ProductService().count_amazon_product() + 1) * 900))
